...
const DEFAULT_EXPENSE_CATEGORIES: Category[] = [
  ...
  { id: '3', name: 'Hoa quả', icon: 'fruit-cherries', color: '#00CED1', count: 3 },
  ...
  { id: '8', name: 'Rượu và đồ uống', icon: 'glass-wine', color: '#BA55D3', count: 2 },
  { id: '9', name: 'Chi phí điện nước', icon: 'water', color: '#4682B4', count: 3 },
...
const DEFAULT_INCOME_CATEGORIES: Category[] = [
  { id: 'i1', name: 'Lương', icon: 'cash-multiple', color: '#4CAF50', count: 12 },
  ...
  { id: 'i9', name: 'Khác', icon: 'dots-horizontal', color: '#9E9E9E', count: 1 },
];
      ...
      try {
        ...
        if (storedExpense) {
          ...
          if (Array.isArray(parsedExpense) && parsedExpense.length > 0) {
            ...
            await AsyncStorage.setItem('expenseCategories', JSON.stringify(DEFAULT_EXPENSE_CATEGORIES));
          }
        } else {
          await AsyncStorage.setItem('expenseCategories', JSON.stringify(DEFAULT_EXPENSE_CATEGORIES));
        }
        ...
        if (storedIncome) {
          ...
          if (Array.isArray(parsedIncome) && parsedIncome.length > 0) {
            ...
            await AsyncStorage.setItem('incomeCategories', JSON.stringify(DEFAULT_INCOME_CATEGORIES));
          }
        } else {
          await AsyncStorage.setItem('incomeCategories', JSON.stringify(DEFAULT_INCOME_CATEGORIES));
        }
    ...
    if (route.params?.updatedCategories) {
      ...
      if (Array.isArray(updatedCategories)) {
        if (activeTab === 'expense') {
          ...
          AsyncStorage.setItem('expenseCategories', JSON.stringify(updatedCategories));
        } else {
          setIncomeCategories(updatedCategories);
          AsyncStorage.setItem('incomeCategories', JSON.stringify(updatedCategories));
        }
      }
    }
      ...
      <View style={styles.tabContainer}>
        ...
        >
          <Text style={[
            ...
            CHI TIÊU({expenseCategories.reduce((sum, cat) => sum + cat.count, 0)})
          </Text>
        </TouchableOpacity>
        <TouchableOpacity 
